...
INSERT INTO prv_Pipeline(pipelineName) VALUES ("%s")''' % name)
        ...
        cursor.execute('''
INSERT INTO prv_cnf_Pipeline(pipelineId, validityBegin, validityEnd, notes)
VALUES (%d, "%s", "%s", "Initial version of %s")''' % \
                           (pipeId, self._currentTime, self._infinity, name))
...
VALUES (%d, "%s", "%s", "%s")''' % (taskId, self._currentTime, self._infinity,
                                    task.gitSHA))
                taskCnfId = cursor.lastrowid
                for k in task.paramKVDict:
                    cursor.execute('''
INSERT INTO prv_cnf_Task_KVParams(taskCnfId, theKey, theValue)
VALUES (%i, "%s", "%s")''' % (taskCnfId, k, task.paramKVDict[k]))
            cursor.execute('''
INSERT INTO prv_cnf_Pipeline_Tasks(pipelineCnfId, taskId, taskPosition)
VALUES ("%s", "%s", %d)''' % (pipeCnfId, taskId, taskPos))
            taskPos += 1

        # register tables pipeline produces
        for table in tables:
            cursor.execute('''
INSERT INTO prv_TableToPipeline(tableName, pipelineId) VALUES ("%s", "%s")''' % \
                               (table, pipeId))
            tToPId = cursor.lastrowid
            cursor.execute('''
INSERT INTO prv_cnf_TableToPipeline(tableToPipelineId, validityBegin,
validityEnd) VALUES ("%s", "%s", "%s")''' % \
    ...
    def registerNode(self, cursor, name, ip, os, cores, ram):
        ...
        """
        cursor.execute('INSERT INTO prv_Node(nodeName) VALUES ("%s")' % \
                             (name))
        nodeId = cursor.lastrowid
        cursor.execute('''
INSERT INTO prv_cnf_Node(nodeId, validityBegin, validityEnd, ip, os, cores, ram)
...
VALUES (%i, "%s", "%s")''' % (taskCnfId, k, task.paramKVDict[k]))
    ...
    def registerTaskExecution(self, cursor, taskName, nodeId, sceGroupId):
        ...
        """
        cursor.execute('''
SELECT taskId FROM prv_Task WHERE taskName='%s' ''' % taskName)
        row = cursor.fetchone()
        if row is None:
...
VALUES ('%s', %i, '%s')''' % (taskId, nodeId, self._currentTime))
